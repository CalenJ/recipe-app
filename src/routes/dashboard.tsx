// src/routes/dashboard.tsx (UPDATED CODE)
...
// --- NEW AUTH IMPORTS START ---
import { useNavigate } from "@solidjs/router"; // <-- Need useNavigate for redirects
import { useAuth } from "~/components/auth/AuthProvider"; // <-- Import useAuth
// --- NEW AUTH IMPORTS END ---
...
export default function Dashboard() {
    ...
    const { session, loading } = useAuth(); // Access the global auth state
...
    const [username, setUsername] = createSignal("")
    ...
    // --- ROUTE GUARD EFFECT (REPLACES OLD onMount AUTH CHECK) ---
    createEffect(() => {
        // Redirect to login if loading is done AND there is no session
        if (!loading() && !session()) {
...
    onMount(async () => {
        // Use the session from the useAuth hook instead of calling supabase.auth.getUser() again
...
      }
        // NOTE: The previous code called supabase.auth.getUser() which is redundant 
        // if using the AuthProvider. The onMount code now uses the session() signal.
    });
...
// ... MainArea component remains the same ...
